...
public class EditTemplateController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Platform.runLater(this::loadDrug);
    }
    ...
    private void handleSaveTemplate(ActionEvent event) {
        if (templateDrugList.isEmpty()) {
            ...
            alert.setHeaderText("You didn't add any medicines");
            alert.show();
        } else if (tfTemplateName.getText() == null || tfTemplateName.getText().length() == 0) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Add a template name");
            ...
            if (templateGetway.update(template, templateDrugList)) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Template updated");
                alert.setHeaderText("Template updated");
                alert.setContentText("Template updated");
                alert.show();
            }
        }
    }
    ...
    public void loadTemplateDetails(int templateId) {
        ...
        tfTemplateNote.setText(template.getNote());
